...
const int PlmnWideGeographicalScope = 1;
const int LocationAreaWideGeographicalScope = 2;
...
const char GeoFenceAttentionRequiredProperty[] =
...
const QStringList TableStatements = {
    QStringLiteral(
        ...
        ")"),
    QStringLiteral(
        "CREATE TABLE IF NOT EXISTS cellbroadcast_messages ("
        "    id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "    alert_id INTEGER NOT NULL"
        "        REFERENCES cellbroadcast_alerts(id) ON DELETE CASCADE,"
...
const QStringList IndexStatements = {
    QStringLiteral(
        "CREATE INDEX IF NOT EXISTS cellbroadcast_alert_state"
        "    ON cellbroadcast_alerts(state, created_at, id)"),
    QStringLiteral(
        "CREATE INDEX IF NOT EXISTS cellbroadcast_message_alert"
...
bool CellBroadcastStore::reconcileQueue()
{
    ...
    for (quint64 id : extraActive) {
        ...
        update.prepare(QStringLiteral(
                "UPDATE cellbroadcast_alerts SET state = 0 WHERE id = ?"));
    ...
    if (!haveActive) {
        ...
        if (!pending.exec(QStringLiteral(
                "SELECT id"
                "  FROM cellbroadcast_alerts"
                " WHERE state = 0"
                " ORDER BY created_at, id"
        ...
        if (pending.next()) {
            ...
            promote.prepare(QStringLiteral(
                    "UPDATE cellbroadcast_alerts SET state = 1 WHERE id = ?"));
                                         ...
                                         const QString &preserveLogicalKey)
{
    ...
    query.prepare(QStringLiteral(
            "DELETE FROM cellbroadcast_alerts"
            " WHERE state = ?"
            "   AND (? = 1 OR logical_key <> ?)"
            "   AND COALESCE((SELECT MAX(last_received_at)"
            "                   FROM cellbroadcast_messages"
            "                  WHERE alert_id = cellbroadcast_alerts.id), 0) < ?"));
        ...
        const QString &text, const QVariantMap &properties)
{
    ...
    // DBGF lookup already excludes suppressed records after 24 hours. If no
    // broadcasts arrive, the table cannot grow, so cleanup at startup and when
    // storing the next broadcast is sufficient and needs no timer.
    ...
    existing.prepare(QStringLiteral(
            "SELECT a.id, a.state, a.current_message_id,"
            "       m.update_number, m.properties_json, a.geo_state,"
            "       (SELECT MAX(last_received_at)"
            "          FROM cellbroadcast_messages"
            "         WHERE alert_id = a.id),"
            "       m.boot_id"
            "  FROM cellbroadcast_alerts a"
            "  LEFT JOIN cellbroadcast_messages m"
            "    ON m.id = a.current_message_id"
            " WHERE a.logical_key = ?"));
    ...
    if (haveExisting) {
        ...
        currentGeoFenceAttentionRequired = currentProperties.value(
                    QLatin1String(GeoFenceAttentionRequiredProperty)).toBool();
    ...
    if (!haveExisting) {
        ...
        insertAlert.prepare(QStringLiteral(
                "INSERT INTO cellbroadcast_alerts ("
                "    logical_key, category_id, title, alert_level,"
                "    attention_policy, display_policy, state,"
                "    created_at, updated_at, geo_state"
                ") VALUES ("
                "    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?"
                ")"));
    ...
    if (preferredLanguage && candidateRawGeometry == currentRawGeometry
        ...
        jsonProperties.insert(QLatin1String(GeoFenceGeometriesProperty),
                              currentGeometry);
        const char *preservedKeys[] = {
            "MaximumWaitTime",
            GeoFenceDeadlineProperty,
            GeoFenceAttentionRequiredProperty
        };
    ...
    if (needsGeoCheck(jsonProperties)) {
        if (valueString(jsonProperties, GeoFenceGeometriesProperty).isEmpty()) {
            jsonProperties.insert(QLatin1String(GeoFenceGeometriesProperty),
                                  valueString(jsonProperties, "Geometries"));
        }
        if (jsonProperties.value(
                    QLatin1String(GeoFenceDeadlineProperty)).toLongLong() <= 0) {
            jsonProperties.insert(QLatin1String(GeoFenceDeadlineProperty),
                                  newGeoFenceDeadline(jsonProperties, timestamp));
        }
        if (candidateNeedsGeoCheck
                || (preferredLanguage && currentGeoFenceAttentionRequired)) {
            jsonProperties.insert(QLatin1String(GeoFenceAttentionRequiredProperty),
                                  geoFenceAttentionRequired);
        }
    }
    // Warning Area Coordinates is an encoded 3GPP information element. Keep
    // its raw bytes in a BLOB; parsed geometry remains in properties_json.
    const QByteArray wac = jsonProperties.take(
                QLatin1String(WarningAreaCoordinatesProperty)).toByteArray();
    ...
    insertMessage.prepare(QStringLiteral(
            "INSERT INTO cellbroadcast_messages ("
            "    alert_id, version_key, message_identifier, serial_number,"
            "    geographical_scope, message_code, update_number, dcs,"
            "    language, language_role, page_count, body, properties_json,"
            "    wac, received_at, last_received_at, supersedes_id, boot_id"
            ") VALUES ("
            "    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?"
            ")"));
...
QVariantMap CellBroadcastStore::readAlert(quint64 id) const
{
    ...
    QSqlQuery query(QSqlDatabase::database(m_connectionName));
    query.prepare(QStringLiteral(
            "SELECT a.id, a.category_id, a.title, a.alert_level,"
            "       a.attention_policy, a.display_policy, a.state, a.created_at,"
            "       a.updated_at, a.geo_state, a.acknowledged_at, a.silenced_at,"
            "       m.id, m.body, m.message_identifier, m.serial_number,"
            "       m.geographical_scope, m.message_code, m.update_number, m.dcs,"
            "       m.language, m.language_role, m.page_count, m.properties_json,"
            "       m.wac, m.received_at, m.last_received_at, m.receipt_count,"
            "       m.supersedes_id"
            "  FROM cellbroadcast_alerts a"
            "  JOIN cellbroadcast_messages m"
            "    ON m.id = a.current_message_id"
            " WHERE a.id = ?"));
    ...
    result.insert(QLatin1String(WarningAreaCoordinatesProperty), query.value(24));
...
bool CellBroadcastStore::acknowledge(quint64 id)
{
    ...
    acknowledge.prepare(QStringLiteral(
            "UPDATE cellbroadcast_alerts"
            "   SET state = 2,"
            "       updated_at = ?,"
            "       acknowledged_at = ?"
        ...
        quint64 id, bool display, const QString &geoState)
{
    ...
    current.prepare(QStringLiteral(
            "SELECT a.state, m.properties_json"
            "  FROM cellbroadcast_alerts a"
            "  JOIN cellbroadcast_messages m"
            "    ON m.id = a.current_message_id"
            " WHERE a.id = ?"));
        ...
        const QString &referenceList, const QVariantMap &triggerProperties) const
{
    ...
    for (const QString &reference : references) {
        ...
        QSqlQuery query(QSqlDatabase::database(m_connectionName));
        query.prepare(QStringLiteral(
                "SELECT DISTINCT a.id, m.properties_json"
                "  FROM cellbroadcast_alerts a"
                "  JOIN cellbroadcast_messages cm"
                "    ON cm.id = a.current_message_id"
                "  JOIN cellbroadcast_messages m"
                "    ON m.alert_id = a.id"
                " WHERE a.state IN (0, 3, 4)"
                "   AND m.message_identifier = ?"
                "   AND m.serial_number = ?"
                "   AND m.last_received_at >= ?"
    ...
    for (const QVariant &value : referenced) {
        ...
        if (alert.value(QStringLiteral("State")).toInt() == AwaitingLocation) {
            const qint64 existingDeadline = alert.value(
                        QLatin1String(GeoFenceDeadlineProperty)).toLongLong();
            if (existingDeadline > 0) {
                alertDeadline = existingDeadline;
            }
        }
        properties.insert(QLatin1String(GeoFenceGeometriesProperty), geometries);
        properties.insert(QLatin1String(GeoFenceDeadlineProperty), alertDeadline);
...
bool CellBroadcastStore::remove(quint64 id)
{
    ...
    QSqlQuery query(QSqlDatabase::database(m_connectionName));
    query.prepare(QStringLiteral(
            "DELETE FROM cellbroadcast_alerts WHERE id = ? AND state = 2"));
    query.addBindValue(id);
...
int CellBroadcastStore::clearHistory()
{
    ...
    QSqlQuery query(QSqlDatabase::database(m_connectionName));
    if (!query.exec(QStringLiteral(
            "DELETE FROM cellbroadcast_alerts WHERE state = 2"))) {
        setError(query.lastError().text());
        return 0;
    }
...
void CellBroadcastStore::setError(const QString &errorString) const
{
    ...
    qWarning() << "Cell Broadcast storage error:" << errorString;
}
